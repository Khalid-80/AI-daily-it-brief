...
def collect_items(config: dict) -> list[dict]:
    ...
    for feed in config["feeds"]:
        ...
        print(f"[info] {feed['name']}: {raw_count} raw entr(y/ies)", file=sys.stderr)
        if raw_count == 0 and getattr(parsed, "bozo", 0):
            print(f"[warn] {feed['name']}: feed may be malformed — bozo_exception={getattr(parsed, 'bozo_exception', '')}", file=sys.stderr)
